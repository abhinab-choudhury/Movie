      ...
      <section className="h-fit max-w-[1920px] max-h-[840px] flex flex-col md:flex-row justify-start backdrop-blur-sm p-[8%]">
        <div className='flex flex-col gap-7 justify-start align-middle md:w-[50%] my-8'>
          ...
          <div className='noto-sans-regular rounded-xl text-balance md:text-lg'>
            Dive into the world of cinema with Movie-Sovi, your go to destination for
            discovering and sharing movie reviews. Whether you&apos;re looking for honest
            opinions, hidden gems, or the next blockbuster to watch, Movie-Sovi is here to
            connect movie lovers from all walks of life.
          </div>
          <div className='flex justify-start'>
            <Button fullWidth variant="gradient" size="lg" className="hidden lg:inline-block w-auto">
              <span>Sign up to know More</span>
            </Button>
          </div>
        </div>
        <div className='md:w-[50%] mx-9'>
          <img className='ml-auto rounded-2xl mx-10 my-5 object-cover w-[100%] md:w-[380px]' src={hero1} alt="movie-scene" />
        </div>
      </section>
          ...
          <div className="my-10">
            {
              faqs.map((question, idx) => (
                <Accordion key={idx} open={open === (idx)}>
                  ...
                  <AccordionBody className='text-balance md:text-lg'>
                    {question.ans}
                  </AccordionBody>
                </Accordion>
              ))
